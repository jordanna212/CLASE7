...
function setup() {
   ...
   noLoop();
   noStroke(); //esto es para que los circulos no aparezcan con circulo negro
}
...
function draw() {
   ...
   for (var c = 0; c < data.getRowCount(); c++) {
      if ((data.get(c, 1) == 0) && (data.get(c, 5) == "female")) {
         muerta++
      } else if ((data.get(c, 1) == 0) && (data.get(c, 5) == "male")){
         muerto++
       }
    }
...
//hombres en x clase y mujeres en clases // tantos mujeres murieron
